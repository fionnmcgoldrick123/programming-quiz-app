...
eliminating training and serving skew. 

Hastie, Tibshirani, and Friedman~\cite{hastie2009esl} provide the statistical learning foundation for the estimators used, and Goodfellow, Bengio, and Courville~\cite{goodfellow2016deep} offer a reference should future iterations replace TF-IDF with a learned representation.

The \texttt{ColumnTransformer} class from scikit-learn~\cite{pedregosa2011} deserves particular attention because it is the component that makes it possible to apply fundamentally different transformations to different feature subsets within a single unified pipeline. In the difficulty classifier, the training matrix contains both dense numeric features derived from submission statistics and a single raw text column representing the problem description. Without \texttt{ColumnTransformer}, these would need to be preprocessed separately and then concatenated manually at each stage of experimentation, introducing opportunities for index misalignment and inconsistent imputation. By declaring a numeric branch (\texttt{SimpleImputer} followed by \texttt{StandardScaler}) and a text branch (\texttt{FeatureUnion} of two \texttt{TfidfVectorizer} instances) inside a single transformer, the entire preprocessing graph is captured in one serialisable object. When \texttt{joblib.dump}~\cite{joblib2025} writes the trained pipeline to disk, the fitted vocabulary, IDF weights, imputation medians, and scaling parameters are all preserved together. When the same object is loaded at inference time by \texttt{difficulty\_service.py}, there is no possibility of applying the wrong vocabulary or the wrong scaling factors, because there is no separate step where they could diverge.

The \texttt{joblib} library itself warrants more than a passing mention. Its \texttt{dump} and \texttt{load} functions use a protocol based on Python's \texttt{pickle} serialisation format but with additional support for memory mapping large NumPy arrays~\cite{numpy2020}. For the difficulty model, which includes a fitted \texttt{XGBoost}~\cite{chen2016xgboost} ensemble with hundreds of trees alongside the full TF-IDF vocabulary matrices, the serialised \texttt{.pkl} file is a non-trivial binary artefact. The memory mapping capability means that on systems with sufficient memory, the array components of the loaded model can be accessed without a full copy into RAM, which reduces the startup overhead of loading the model into the Uvicorn worker process. In practice, for the dataset sizes involved in this project, the full load takes under two seconds on modest hardware and is therefore not a bottleneck. However, the architectural decision to load the model once at module import time rather than on every request means this cost is paid exactly once per server process lifetime, regardless of how many quiz generation requests subsequently arrive.

The \texttt{pandas}~\cite{mckinney2010pandas} library plays a narrower but equally necessary role. The difficulty classifier's inference function, \texttt{predict\_difficulty\_for\_question}, must construct a single row \texttt{DataFrame} whose column names exactly match those seen during training. This is the canonical way to pass data to a scikit-learn pipeline that contains a \texttt{ColumnTransformer}, because the transformer selects its input columns by name rather than by position. Using a raw NumPy array would require the caller to know and maintain the exact column ordering, which is fragile and error prone. The \texttt{DataFrame} interface makes the column names explicit at the call site, so any mismatch between the feature names produced by inference code and those expected by the trained transformer raises an immediate and informative error rather than silently producing a wrong prediction.